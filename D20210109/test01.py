...
def runAllImageSimilaryFun(para1):
    ...
    if para1.startswith("http"):
        ...
        # 通过imread方法直接读取物理路径
        img = Image.open(para1)  #imread()读取后是ndarray
        # img2 = cv2.imread(para2)   #Image.open是PIL格式
        # img = np.asarray(Image.open(para1))#PIL格式转ndarray
        # img2 = np.asarray(Image.open(para2))
        height = img.size[0]
        width = img.size[1]
        list1 = (width, height)
        temp1 = min(list1)
        plot1=[]
        temp=[]
        for i in np.arange(1, 7 + 0.01, 0.05):
            temp2 = temp1 * i
            temp.append(i)
            img_resized = img.resize((temp1, round(temp2)), Image.ANTIALIAS)  # resize image with high-quality
            img1 = np.asarray(img)#PIL格式转ndarray
            img2 = np.asarray(img_resized)
