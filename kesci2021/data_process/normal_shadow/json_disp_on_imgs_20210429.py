...
if __name__ == '__main__':
  ...
  ju
    ...
    img = test_json_with_shadow['images']
    images = []
    ...
    csv_file=open('0.5048.csv','w')
    csv_file.write("name,image_id,confidence,xmin,ymin,xmax,ymax\n")
    imgid2anno = {}
    imgid2n_anno = {}
    imgid2name = {}
    n_name2_imgid = {}
    for imageinfo in test_json_with_shadow['images']:
        imgid = imageinfo['id']
    ...
    # for img_id_normal, anno_normal_one_image in imgid2n_anno.items():  # normal
    #     for img_id_shadow, anno_shadow_one_image in imgid2anno.items():
    #         for anno_normal in anno_normal_one_image:
    #             for anno_shadow in anno_shadow_one_image:
    #                 m_iou = compute_IOU(anno_shadow['bbox'], anno_normal['bbox'])
    #                 a=1
    #                 # if m_iou >= 0.85 and m_iou < 1:
    #                 #     anno_shadow['category_id'] = anno_normal['category_id']

    for imgid, annos in imgid2anno.items():  # shadow
        if imgid in imgid2n_anno.keys():   # normal
            ...
            for n in annon:
                ...
                # annon=imgid2n_anno[imgid]
            # # for i,a_s in enumerate(annos):
            # for j,a_n in enumerate(annon):
            #


            #         rec1=a_s['bbox']
            #         rec2=a_n['bbox']
            #         IOU=my_iou(rec1,rec2)
            #         a = 1
                    # if IOU>0.8:
                    #     annon[j]['category_id']=annos[i]['category_id']
            ...
            for anno in annos:  # shadow
                ...
                # image_name = imgid2name[imgid]
                # image_id = image_name.split('.')[0]    # + '.xml'
                if confidence >= score_threshold_with_shadow:
                    cv2.rectangle(src, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (255, 0, 0), n_w)
                    cv2.putText(src, '{}|{:.3f}'.format(class_name, confidence),
                                (int(xmin - 24), int(ymax) + 30),
                                cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 0, 0), n_w)
